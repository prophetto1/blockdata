...
// UI-only mockup to cue development direction:
// - Shows runs progressing (as if a worker is connected)
// - Still marks worker state explicitly (dev mode / local worker)
...
const RUNS = [
  {
    ...
    status: "processing",
    progress: { pending: 62, claimed: 3, complete: 89, failed: 2, total: 156 },
    created_at: "2026-02-02",
  },
  {
    run_id: "a2f9d1c8-…",
    schema_ref: "strunk_18_v1",
    model: "ollama:llama3.2:3b",
    status: "complete",
    progress: { pending: 0, claimed: 0, complete: 156, failed: 0, total: 156 },
    created_at: "2026-02-02",
  },
];
...
function Btn({ children, variant = "primary", className = "", ...props }) {
  ...
  return (
    <button
      {...props}
      className={cx("px-3 py-2 rounded-lg text-sm font-semibold transition-colors", cls, className)}
    >
      {children}
    </button>
  );
...
function RunRow({ run }) {
  const tone =
    run.status === "complete" ? "good" : run.status === "processing" ? "warn" : run.status === "failed" ? "bad" : "neutral";
  const label =
    ...
    <div className="bg-zinc-900/30 border border-zinc-800 rounded-xl p-3">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="mt-1 text-xs text-zinc-500">{run.model}</div>
      ...
      <div className="mt-3 flex gap-2 justify-end">
        <Btn variant="ghost">Export overlay JSONL</Btn>
      </div>
    </div>
  );
}

export default function MdAnnotateMockupV3() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      <div className="max-w-6xl mx-auto px-6 py-6">
        <div className="mb-4 bg-violet-500/10 border border-violet-500/30 rounded-2xl p-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-sm font-bold text-violet-200">Dev mode</div>
              <div className="text-xs text-zinc-300 mt-1">
                This environment is private. The UI shows the intended direction: runs progress via a worker loop (ex:
                local Ollama).
              </div>
            </div>
            <div className="flex gap-2">
              <Btn variant="ghost">Worker: connected</Btn>
              <Btn>Start new run →</Btn>
            </div>
          </div>
        </div>
        ...
        <div className="mt-5 grid grid-cols-12 gap-4">
          ...
          <div className="col-span-3 bg-zinc-900/20 border border-zinc-800 rounded-2xl p-4">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-bold text-zinc-200">Runs</h2>
              <Pill tone="warn">dev</Pill>
            </div>
        ...
        <div className="mt-6 text-xs text-zinc-600">
          This page cues the intended direction: runs progress in parallel via a worker loop (claim → model → complete)
          while export remains available for blocks and runs.
        </div>
      </div>
    </div>
  );
