...
function buildHarnessDesignerStorageKey(projectId: string) {
  return `agchain.harness.designer.graph.v1.${projectId}`;
}

export default function AgchainHarnessDesignerPage() {
  ...
  if (scopeState.kind === 'error') {
    return (
      <AgchainPageFrame className="gap-6 py-8">
        <AgchainEmptyState
          title="AGChain harness designer unavailable"
    ...
    return (
      <AgchainPageFrame className="gap-6 py-8">
        <AgchainEmptyState
          ...
          description="Harness designer authoring is scoped to the selected AGChain project. Pick a project from the registry before working in this surface."
          action={(
            <Link
  ...
  const storageKey = buildHarnessDesignerStorageKey(scopeState.focusedProject.project_id);
    ...
    <AgchainPageFrame className="gap-0 px-0 pb-0">
      <div className="px-4">
        <ShellPageHeader
          ...
          description={`Compose harness objects, linked skills, and prompt relationships for ${projectName}. This route gives AGChain a dedicated home for harness authoring without collapsing it back into the older workflow naming.`}
        />
      </div>
      ...
      <div className="min-h-0 flex-1 px-2 pb-2">
        <WorkflowEditorSurface
          ...
          canvasTestId="harness-designer-canvas"
        />
      </div>
    </AgchainPageFrame>
  );
