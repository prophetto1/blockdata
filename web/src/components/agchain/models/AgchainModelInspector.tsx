...
import { ModelTargetFormFields } from './AgchainModelsToolbar';
...
function AgchainModelInspectorSession({
  ...
  onConnect,
  onDisconnect,
}: AgchainModelInspectorSessionProps) {
  const [createOpen, setCreateOpen] = useState(false);
  const [createDraft, setCreateDraft] = useState<ModelTargetDraft>(() =>
    buildCreateDraftForProvider(providers, providerRow.provider_slug),
  );
  ...
  const providerDefinition = providerRow.provider_definition;
      ...
      <section className="rounded-2xl border border-border/70 bg-card/60 p-4">
        ...
        {createOpen ? (
          <div className="mt-4 rounded-2xl border border-border/70 bg-muted/20 p-4">
            ...
            <div className="mt-4">
              <ModelTargetFormFields draft={createDraft} providers={providers} onChange={setCreateDraft} providerLocked />
            </div>
          </div>
        ) : null}
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    >
                      ...
                      <p className="mt-1 truncate font-mono text-xs text-muted-foreground">{target.qualified_model}</p>
                    </button>
                  ...
                  {isEditing && editDraft ? (
                    <div className="mt-4 rounded-2xl border border-border/70 bg-muted/20 p-4">
                      ...
                      <div className="mt-4">
                        <ModelTargetFormFields draft={editDraft} providers={providers} onChange={setEditDraft} providerLocked />
                      </div>
                    </div>
                  ) : null}
                      ...
                      {loading ? (
                        ...
                        <div className="mt-3 space-y-3">
                          {recentHealthChecks.map((check) => (
                            <article key={check.health_check_id} className="rounded-xl border border-border/70 bg-card px-4 py-3">
                              <div className="flex items-center justify-between gap-3">
