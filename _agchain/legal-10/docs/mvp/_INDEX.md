...
1. `M1-buildtime-packaging-sealing-dev-brief.md` — **MOST AUTHORITATIVE**. Frozen decisions on bundle structure, EU schema, benchmark schema, sealing (manifest.json + signature.json). Overrides other docs.
...
### MVP FDQ Variants
These are 3-step-specific extracts of the canonical FDQs in `../fdq/`:
6. `fdq-01-ka-sc.md` — d1 KA-SC spec (identical to canonical `fdq/01-ka-sc.md`)
7. `fdq-02-irac-without-rp.md` — d2 IRAC closed-book
8. `fdq-03-irac-with-rp.md` — j3 IRAC open-book
...
`M1-buildtime-packaging-sealing-dev-brief.md` is the highest-authority document for the MVP. When other docs conflict with it on bundle layout, schema shapes, or sealing requirements — M1 wins.
