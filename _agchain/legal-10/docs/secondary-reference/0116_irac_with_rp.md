...
**Type:** Judge-scored
**Chain Position:** As of now, this question type is always last question in the chain. Normally, question types dont have a fixed position in the chain, but this question is designed to be presented last.
(For example, in AG8 --> j8, AG0 --> j10, A25 --> j25)

---

## Description

Irac with RP presents the Evaluated Model with a research pack (p#.json). The model uses the anchor text and the research pack to write an IRAC. It is provided with instructions on how to write the IRAC and an overview of the grading criteria.
It is conducted immediately after the Irac without RP question
