...
def extract_citations(text: str) -> list[str]:
    ...
    try:
        ...
        # Extract the literal US citation from the eyecite object (handling repr leakage)
        pattern = r"\d{1,3}\s+U\.?\s*S\.?\s+\d{1,4}"
        for c in eyecite_results:
            m = re.search(pattern, str(c), re.IGNORECASE)
            if m:
                citations.append(m.group(0).strip())
        
        # Fall through to deduplication at the bottom
