...
from core.runtime.results import StepResult


class Step(ABC):
    """Abstract base class for chain steps.

    Each step in the (legacy) chain implements this interface (e.g., S1-S10).
    Canonical step naming in current specs may use `d*` / `j*` identifiers.
    ...
    def __init__(self) -> None:
        # Contract parsing diagnostics (optional, populated by subclasses)
        self._last_validation_errors: list[Any] = []
    ...
    @abstractmethod
    def prompt(self, ctx: ChainContext) -> str | tuple[str, Any, dict]:
        ...
        Returns:
            Prompt string or (prompt_text, prompt_obj, prompt_meta) tuple
        """
        pass
    ...
    def get_last_validation_errors(self) -> list[Any]:
        """Return validation errors from the last parse attempt."""
