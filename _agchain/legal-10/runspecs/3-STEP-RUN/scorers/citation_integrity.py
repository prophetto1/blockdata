"""Deterministic citation-integrity scorer (step-ID agnostic).

Compares model-cited authorities in two IRAC outputs (closed-book + open-book)
against the build-time anchor citation inventory and RP subset.

Canonical step IDs: d9 (closed-book) / j10 (open-book).
Back-compat aliases: irac_no_rp / irac_with_rp (for 3-step MVP d2/j3).
"""
...
def score_citation_integrity(
    *,
    ...
    """Deterministically score citation usage in the two IRAC outputs.

    Canonical params: irac_d9 (closed-book), irac_j10 (open-book).
    Back-compat aliases: irac_no_rp, irac_with_rp (for 3-step MVP d2/j3).
    ...
    if irac_d9 is None or irac_j10 is None:
        raise ValueError("score_citation_integrity requires IRAC outputs for both closed-book and open-book steps")
    ...
    # j10 (open-book) may cite from anchor inventory OR the research pack subset
    j10_valid_inventory = _dedupe_preserve_order([*anchor_inventory_full, *rp_subset])
    validity_j10 = _validity(citations_used_j10, j10_valid_inventory)
    ...
    combined_validity = _validity(combined_used, j10_valid_inventory)
