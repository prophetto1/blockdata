...
async function loadNav() {
  // Force load nav.json always
  const navFile = "nav.json";
  // Removed isScientificVariant logic as Methodology B is deleted
  ...
  for (const base of bases) {
    try {
      const res = await fetch(`${base}/${navFile}`, { cache: "no-store" });
      if (!res.ok) throw new Error(`Failed to load ${navFile}: ${res.status}`);
      return res.json();
    } catch (err) {
      lastErr = err;
    }
  ...
  throw lastErr || new Error(`Failed to load ${navFile}`);
}

function setThemeFromStorage() {
  // TEMPORARY: Force dark mode until light mode styling is fixed across all pages
  document.documentElement.classList.add("dark");
  // Original code (restore when light mode is ready):
...
function applyBodyStyles() {
  ...
  const classes = [
    ...
    "dark:text-[#d4d4d4]",
    "transition-colors",
    "font-serif", /* Default to Merriweather */
  ];
...
function renderSideNavFlat(side, items, layoutMode = "default") {
  ...
  for (const item of items || []) {
    ...
    // Robust Metadata Lookup (handle potential whitespace or casing issues)
    let meta = NAV_METADATA[item.title];
    if (!meta && item.title.trim() !== item.title)
      meta = NAV_METADATA[item.title.trim()];
    if (!meta) meta = { color: "#171717", bg: "rgba(0,0,0,0.05)", icon: "" };
    ...
    if (!hasChildren) {
      ...
      if (isResearch && meta.icon) {
        ...
        if (isActive) {
          titleText.style.color = meta.color;
          titleText.classList.add("font-bold");
        } else {
          // Ensure default color if not active, or let CSS handle it?
          // CSS handles it via class, but explicit override ensures parity with V1 fix if needed.
          // Let's stick to CSS defaults for inactive unless V1 did otherwise.
          // V1 fix did: titleLink.style.color = "#4b5563";
          titleText.style.color = "#4b5563";
          if (document.documentElement.classList.contains("dark")) {
            titleText.style.color = "#9ca3af";
          }
        }
    ...
    if (isResearch) {
      ...
      for (const child of item.children) {
        ...
        if (isActive) {
          a.style.color = meta.color; // Use pillar color for active link text
        }
...
async function mountShell() {
  ...
  const style = document.createElement("style");
  style.textContent = `
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600&family=Outfit:wght@600;700&family=Source+Serif+4:ital,opsz,wght@0,8..60,400;0,8..60,600;1,8..60,400&family=Fira+Code:wght@400;500;600&family=Merriweather:ital,wght@0,300;0,400;0,700;1,300&display=swap');
    ...
    *::-webkit-scrollbar {
    ...
    #l10-page { 
      ...
      max-width: none; /* Full width default */
    }
    ...
    body.l10-codex-mode #l10-page { 
      ...
      color: #a3a3a3 !important;     /* Muted grey (Exact match to slides) */
      font-weight: 300;
    ...
    body.l10-codex-mode #l10-page .research-title { 
      ...
      border-bottom: 1px solid #262626 !important; /* border-neutral-800 match */
    ...
    body.l10-codex-mode #l10-page h4 { 
      ...
      color: #737373 !important; /* neutral-500 approx */
    ...
    body.l10-codex-mode #l10-page li {
        ...
        color: #a3a3a3 !important; /* neutral-400 approx */
    ...
    body.l10-codex-mode td {
        ...
        border-color: #262626 !important; /* Dark mode slide border */
    }
    ...
    .light h1 { border-bottom-color: #e5e5e5; }
    ...
    .light h2 { color: #525252 !important; border-bottom-color: #d4d4d4; }
    ...
    .light h3 { color: #171717 !important; }
    ...
    .dark [data-l10-layout="research"] .sidebar-v2-header-text { color: #f3f4f6; }
    ...
    .dark [data-l10-layout="research"] .sidebar-v2-link { color: #9ca3af; }
  ...
  // Scope Codex aesthetic (Dense/Mono/900px) to Methods & Slides pages only.
  const isCodexPage = window.location.pathname.includes("/methods/") || window.location.pathname.includes("/slides/");
  if (isCodexPage) {
  ...
  drawerBrand.className = "font-bold text-lg dark:text-white";
  drawerBrand.textContent = nav.brand?.title || "Legal-10";
  ...
  drawerDevLink.href = normalizeHref("/developers.html");
  ...
  brand.className =
    "text-lg font-bold tracking-tight dark:text-white shrink-0 brand-logo";
  brand.textContent = nav.brand?.title || "Legal-10";
  ...
  // TEMPORARY: Hide theme toggle until light mode styling is fixed
  const themeBtn = document.createElement("button");
  themeBtn.type = "button";
  themeBtn.id = "theme-toggle";
  themeBtn.title = "Toggle dark mode";
  themeBtn.className =
    "hidden p-2 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors"; // Added 'hidden' class
  themeBtn.innerHTML = `
  ...
  if (layoutMode === "two-col") {
    ...
    layout.className =
      sidePos === "right"
        ? `w-full ${maxWidth} mx-auto ${SHELL_PX} py-8 grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8`
        : `w-full ${maxWidth} mx-auto ${SHELL_PX} py-8 grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-8`;
    ...
    if (sideMode === "toc") {
      ...
      renderSideNavFlat(side, sideItems, layoutMode);
      ...
      if (sideItems.length > 0) {
        ...
        renderSideNavFlat(drawerSideList, sideItems, layoutMode);
  ...
  leftFoot.innerHTML = `© ${new Date().getFullYear()} <span class="font-bold tracking-tight brand-logo">${
    nav.brand?.title || "Legal-10"
  }</span>`;
  ...
  devLink.href = normalizeHref("/developers.html");
...
function injectPrevNextNav(nav) {
  ...
  const sectionColors = {
    ...
    "Validation & Sanity Checks": "emerald",
    "Roadmap & Legacy": "rose"
  };
  ...
  for (const section of methodologyNav.side) {
    ...
    if (section.href) {
      pages.push({ title: section.title, href: section.href, section: sectionTitle, color });
    }

    // Add children
    if (section.children) {
      for (const child of section.children) {
        pages.push({ title: child.title, href: child.href, section: sectionTitle, color });
      }
    }
  }
  ...
  const currentIndex = pages.findIndex(p => {
    const href = p.href;
    // Direct match
    if (currentPath === href || currentPath.endsWith(href)) return true;
    // Handle /methods/ -> /methods/index.html
    if (currentPath.endsWith("/") && href.endsWith("index.html") && currentPath + "index.html" === href) return true;
    if (currentPath.endsWith("/methods/") && href === "/methods/index.html") return true;
    // Match just the filename
    const filename = href.split("/").pop();
    if (currentPath.endsWith("/" + filename)) return true;
    return false;
  });
...
window.l10ShellReady.catch((err) => {
  ...
  banner.textContent = `Legal-10 shell failed to load: ${String(
    err?.message || err
  )}`;
