---
title: "d1 Known Authority scorer"
sidebar:
  order: 5
---
...
def extract_in_scope_citations(text: str) -> list[str]:
    ...
    In-scope: U.S., F., F.2d/F.3d, F. Supp., F. Supp. 2d/3d. (F.4th is out of scope.)
...
def _get_gt(ground_truth: dict[str, Any], key: str) -> Any:
    # Allow both flat and nested ground-truth shapes.
...
def score_d1_known_authority(
    *,
    ...
    Expected *conceptual* fields (names can be finalized in contracts later):
    - controlling_authority: str (one citation)
    - in_favor: list[str]
    - against: list[str]
    - most_frequent: str (one citation)

    Ground truth is expected to contain the same keys (either top-level or under
    `known_authority`).
    """
    errors: list[str] = []
    ...
    controlling_ok = (
        ...
        and _canonical_key(first_or_none(controlling_pred) or "") == _canonical_key(first_or_none(controlling_true) or "")
    )
    most_frequent_ok = (
        first_or_none(most_frequent_pred) is not None
        and first_or_none(most_frequent_true) is not None
        and _canonical_key(first_or_none(most_frequent_pred) or "") == _canonical_key(first_or_none(most_frequent_true) or "")
    )
    ...
    def f1(pred: list[str], truth: list[str]) -> dict[str, Any]:
        ...
        # Report representative strings (stable order) for debugging.
    ...
    # Score: average of 4 components (2 exact-match, 2 F1).
    score_components = [
