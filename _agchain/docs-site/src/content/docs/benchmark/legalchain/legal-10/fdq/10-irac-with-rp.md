---
title: "FDQ 10: IRAC with Research Pack"
sidebar:
  order: 14
---

# FDQ: j10 — IRAC with Research Pack
...
| **Scoring**         | Judge (MEE rubric; grades both d9 + j10)        |
...
| Prior step artifact: `d_extraction` | disposition/party/holding summary | IRAC scaffold                              |
| Prior step artifact: `d_authority`  | overruled status                  | “Authority status” string                  |

**Boundary:** j10 is the open-book boundary where `p2` is admitted.
...
* Requires completion of d9 in the normal chain flow.
...
* `{disposition}` = `d_extraction.parsed.disposition` (fallback: `"unknown"`)
* `{party_winning}` = `d_extraction.parsed.party_winning` (fallback: `"unknown"`)
* `{holding_summary}` = `d_extraction.parsed.holding_summary` (fallback: `"(Not available)"`)

**Authority status string:**

* If `d_authority.parsed.is_overruled == True`:

  * `This case was OVERRULED by {overruling_case}.`
* Else:

  * `This case remains good law (not overruled).`
...
3. judge grades **both IRACs (d9 + j10) in one call**
