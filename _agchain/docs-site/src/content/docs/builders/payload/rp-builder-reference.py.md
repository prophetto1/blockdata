---
title: "RP builder reference implementation"
sidebar:
  order: 1
---
...
﻿#!/usr/bin/env python3
"""
Stage 4A: Build sealed ResearchPacks (RPv1). Pre-factoring version.

# TODO : Requires refactoring. refer to blockers

Updates required for benchmark-structure-v2.md alignment


Goal:
- Prebuild everything at build-time so runtime Runner does zero retrieval.
- Runner later only verifies manifest + injects payloads at delivery boundaries.
            ...
            selected.append(
                {
                    "source": "SCOTUS",
                    "rank": cite.get("rank"),
                    "normalized_cite": us_cite,  # join key into citation_inventory
                    "usCite": us_cite,
            ...
            selected.append(
                {
                    "source": "CAP",
                    "rank": cite.get("rank"),
                    "normalized_cite": str(normalized_cite),  # join key into citation_inventory
    ...
    *,
    ...
    for sel in (scotus_sel_sorted + cap_sel_sorted):
        ...
        if source == "SCOTUS":
            ...
            label_info = depth_labels_by_key.get((anchor_caseId, join_key, cite_offset), {})
    ...
    if write_manifest:
        ...
        # NOTE: manifest.json is intentionally NOT hashed inside itself.
...
def main() -> None:
    p = argparse.ArgumentParser(description="Stage 4A: Build sealed ResearchPacks (RPv1).")
    p.add_argument("--scdb-path", type=Path, default=DATASETS / "scdb_full_with_text.jsonl")
    p.add_argument("--scotus-ranked-path", type=Path, default=DATASETS / "scotus_citations_ranked.jsonl")
    p.add_argument("--cap-ranked-path", type=Path, default=DATASETS / "cap_citations_ranked.jsonl")
    p.add_argument("--syllabi-path", type=Path, default=DATASETS / "casesumm_syllabi.parquet")
    p.add_argument("--cap-head-matter-path", type=Path, default=DATASETS / "cap_head_matter.jsonl")
    p.add_argument("--citation-inventory-path", type=Path, default=DATASETS / "citation_inventory.parquet")
    p.add_argument("--depth-labels-path", type=Path, default=DATASETS / "citation_depth_labels.parquet")
    ...
    p.add_argument("--limit", type=int, default=0, help="Build first N eligible anchors (0 = all).")
    p.add_argument("--anchor-caseid", action="append", default=[], help="Build only these anchor caseIds (repeatable).")
    p.add_argument("--anchor-us-cite", action="append", default=[], help="Build only these anchor usCites (repeatable).")
