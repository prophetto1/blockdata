...
## 19. Solver vs Scorer — Scope Comparison

*Added 2026-03-31 from direct source analysis.*

### The shared substrate: `TaskState`

Both modules operate on `TaskState`, which holds:
- `messages` — the full chat conversation history
- `output` — the model's final response
- `tools` — tools available to the model
- `store` — a key-value bag for inter-solver communication
- `completed` — whether the task is done
- `target` — the correct answer (read-only)
- `scores` — accumulated scoring results

`TaskState` is the thread connecting both modules, but each uses it differently.

### Solver — *transforms the task toward an answer*
...
A Solver **takes a TaskState and returns a modified TaskState**. It may call `generate()` to invoke the model and extend the message history, or manipulate state without calling the model.

**Scope:** Everything that happens *before* you have a final answer. Solvers are the pipeline.

Solvers can be composed: `chain(prompt_template(...), use_tools(...), generate())` is itself a valid Solver.

### Scorer — *judges the final answer*
...
A Scorer **takes a completed TaskState and a Target (the correct answer), and returns a Score**. It does not modify state — it observes it and renders a verdict.

`Score` contains: `value` (C/I/P/N or numeric), `answer` (what the model produced), `explanation` (why), `metadata`, `history` (edit trail for human raters).

Each scorer is **bundled with metrics** — declared at registration time via `@scorer(metrics=[accuracy(), stderr()])`. Metrics compute aggregate statistics *across* all scores for a task run (`list[SampleScore]`), not on individual scores.

### Difference matrix

| Dimension | Solver | Scorer |
|-----------|--------|--------|
| When it runs | Before/during model invocation | After the pipeline completes |
| What it touches | Full `TaskState` — messages, tools, output | `TaskState` (read) + `Target` |
| What it returns | Modified `TaskState` | `Score` (value + explanation) |
| Can call the model? | Yes — via `generate` parameter | Yes — model-graded scorers use a second LLM |
| Can be composed? | Yes — `chain()`, `fork()` | Yes — `multi_scorer()` |
| Owns metrics? | No | Yes — each scorer declares its aggregate metrics |
| Mutation vs judgment | Mutation | Judgment |

**One-line summary:** A Solver shapes the task state into a model response. A Scorer reads that response against the known target and assigns a verdict plus aggregate metrics.

---

## 20. Required vs Optional Modules for an Eval/Benchmark Environment
...
```
Dataset (Samples)
    → Task (assembles dataset + solver + scorer + config)
        → eval() (orchestrates runs across models)
            → for each Sample:
                → Solver pipeline (chain of solvers transform TaskState)
                    → generate() calls Model (provider-abstracted LLM call)
                → Scorer (reads final TaskState, returns Score)
                → Metrics (aggregate Scores into summary stats)
            → EvalLog (structured record of everything that happened)
                → Recorder (persists log to disk/buffer)
```

### Required — Cannot build a functional eval environment without these

| Module | Why required |
|--------|-------------|
| `_eval/` | Orchestration engine — `eval()`, `Task`, per-sample execution loop, epoch management |
| `model/` | All LLM calls go through here — provider abstraction, `generate()`, chat messages, tool dispatch |
| `solver/` | Task execution pipeline — `Solver` protocol, `TaskState`, `chain()`, `generate()` solver |
| `scorer/` | Judgment layer — `Scorer` protocol, `Score`, `Metric`, built-in metrics (`accuracy`, `mean`, `stderr`) |
| `dataset/` | `Sample` and dataset loading — every evaluation starts with Samples |
| `log/` | Persistent eval artifacts — `EvalLog`, `EvalSample`, recorders, read/write |
| `event/` | Within-sample execution trace — feeds the log recorder and `EvalSample.events` |
| `_util/` | Core runtime infrastructure — registry, async, content types, error types, limits |
| `tool/` (infra) | Tool calling protocol required if any solver uses tools; individual built-ins are optional |
| `util/` (partial) | `Store`, `SandboxEnvironment`, and limit helpers required for most real evals |

### Optional — Secondary or feature-specific

| Module | Classification | When you need it |
|--------|---------------|-----------------|
| `agent/` | Optional | Agent-loop evals (tool-using, multi-step, external agent frameworks) |
| `approval/` | Optional | Safety gating on tool calls in high-risk evals |
| `hooks/` | Optional | Lifecycle callbacks for external integration (observability, dashboards) |
| `analysis/` | Optional now, required for results surface | DataFrame extraction for three-axis comparison queries |
| `_display/` | Optional | Terminal TUI — irrelevant for web platforms |
| `_view/` | Optional | Local log browser — superseded by custom frontend |
| `_cli/` | Optional | CLI — programmatic `eval_async()` is the embedding path |
| `_lfs/` | Optional | Only if datasets are stored in Git LFS repos |

### AGChain axis mapping

The three-axis insight ("which runtime policy makes this model strongest") maps directly onto:

- **Model axis** → `model/_providers/` + `model/_generate_config.py`
- **Benchmark axis** → `_eval/task/task.py` + `dataset/` + `scorer/`
- **Environment Profile axis** → `solver/` (which solver pipeline runs) + `util/SandboxEnvironment` (which execution environment) + `approval/` (which safety policy)

`analysis/` is the module to study when building the results comparison view — the DataFrame extraction layer is exactly the query surface for "show me accuracy × model × environment profile."
