...
def test_convert_callback_payload_includes_blocks(client):
    callback_payloads = []
    ...
    async def fake_send_callback(_url, _secret, payload):
        callback_payloads.append(payload)

    with patch("app.api.routes.conversion.convert", side_effect=fake_convert), \
         patch("app.api.routes.conversion.upload_bytes", side_effect=fake_upload_bytes), \
         patch("app.api.routes.conversion.send_conversion_callback", side_effect=fake_send_callback):
        resp = client.post(
            "/convert",
            json={
                "source_uid": "src-1",
                ...
                "output": {
                    "bucket": "documents",
                    "key": "out/file.md",
                    "signed_upload_url": "https://example.test/upload-md",
                },
                "docling_output": {
                    "bucket": "documents",
    ...
    assert callback_payloads[0]["success"] is True
    assert callback_payloads[0]["blocks"][0]["pointer"] == "#/texts/0"
