...
Below is the missing section: **User-Defined Schemas (the mutable overlay layer)**.

This section answers:
- How a block record combines `immutable` + `user_defined` into one JSON object
- How many JSON objects become one JSONL export
- Why the system is designed as immutable substrate + many schema overlays

It also resolves the "one schema vs many" requirement explicitly:
- One document can have **many** user-defined schemas attached.
- One user-defined schema can be used by **many** documents.

---

## User-Defined Schemas (Overlay Layer)

### 1) What a "user-defined schema" is

A user-defined schema is a reusable contract that defines the shape of **per-block outputs**.

- It does **not** change the immutable substrate.
- It defines the fields that will appear under `user_defined.data` for each block.
- The same schema can be applied to:
  - one document
  - many documents
  - the same document multiple times (via separate runs, if desired)

You already have a concrete example schema artifact in the repo:
- `docs/tests/user-defined/prose-optimizer-v1.schema.json`

That schema is "per-block" and expects output fields like:
- `action` (keep/rewrite/skip_nonprose)
- `final.content` (the rewritten Markdown fragment)
- `checks[]` (small set of rule checks)

### 2) Multi-schema model (document <-> schema is many-to-many)

To satisfy "many schemas per document" and "many documents per schema", the system needs a **binding** concept.

Terminology used throughout this repo:

- **Schema**: the reusable schema definition artifact (stored once).
- **Run**: a specific attachment of a schema to a document (a binding instance).
- **Overlay row**: a per-block output payload for a specific run.

In Phase 2 (planned / scaffolding), this corresponds to:
- `schemas`: one row per user-defined schema definition
- `annotation_runs`: one row per (document, schema) attachment instance
- `block_annotations`: one row per (run, block) payload

Product/UI implication (core requirement):
- A document page must show all schemas attached to that document (runs).
- A schema page must show all documents (runs) that use that schema.

### 3) Canonical record (one block JSON = immutable + user_defined)

The canonical export record is **one JSON object per block** with exactly two top-level keys:

- `immutable`: derived at ingest time from the canonical text and parsing rules
- `user_defined`: derived from a chosen schema attachment (a run) and its overlays
...
```json
{
  "immutable": {
    ...
    "envelope": {
      ...
      "md_uid": "...",
      "source_type": "docx",
      "source_locator": "uploads/{source_uid}/original.docx",
      "md_locator": "converted/{source_uid}/original.md",
      "doc_title": "...",
      "uploaded_at": "...",
  ...
  "user_defined": {
    ...
    "schema_uid": "sha256(canonicalized_schema_json)",
    "data": {
      "action": "rewrite",
      "final": { "content": "..." },
      "checks": [],
...
Notes:
- This doc uses `user_defined` as the canonical name for the overlay.
- Some older text uses `annotation` for the same concept. Treat that as legacy naming; the layer is the same.

### 4) Phase 1 vs Phase 2 export behavior

Phase 1 (immutable-only) still exports the same shape, but keeps `user_defined` inert:

```json
"user_defined": { "schema_ref": null, "schema_uid": null, "data": {} }
```

Phase 2 (overlay) populates `user_defined` by selecting a specific attachment instance:
- `user_defined.schema_ref` / `user_defined.schema_uid` come from the run's schema
- `user_defined.data` comes from the overlay payload for that run and block

### 5) How JSON becomes JSONL

JSONL is newline-delimited JSON:

- One line = one block record JSON object (the shape above)
- The file = all blocks for a document, ordered by `block_index` ascending

Exports must support two primary selectors:

1) Phase 1: `doc_uid` export
   - immutable only, inert `user_defined`

2) Phase 2: `run_id` export
   - immutable + the overlays for that run

This is the mechanical reason JSONL exists:
- It is a portable artifact (streamable, appendable, line-addressable)
- It is friendly to downstream batch tools and pipelines
- It preserves per-block boundaries without requiring a large in-memory JSON array

### 6) Why this design (what it enables)

This is not an arbitrary split. It is the minimal structure that enables:

- **Parallelization**: blocks become stable units of work; multiple workers can operate concurrently without collisions.
- **Multi-lens reuse**: the same immutable block inventory can be annotated under many schemas without re-ingesting.
- **Auditability / provenance**: every derived field can always point back to the immutable block that produced it.
- **Comparability**: different schema outputs can be compared or merged because they share stable `block_uid` join keys.
- **Drift resistance**: schemas and outputs can evolve without mutating the immutable substrate.
