...
Status: Draft (v3.0 = aligned to `docs/product-defining-v2.0/blocks.md` + `docs/product-defining-v2.0/immutable-fields.md`)
...
- `docs/product-defining-v2.0/Defining User-Defined Schemas.md` (use case illustrations: UC1–4 with full schema artifacts and concrete exports)

Primary source material (canonical for v3.0):
- `docs/product-defining-v2.0/blocks.md` (block types + hybrid block extraction requirement)
- `docs/product-defining-v2.0/immutable-fields.md` (vNext immutable field names + deterministic identity rules)
...
- Additional schema examples (10+ artifacts) are defined inline in `Defining User-Defined Schemas.md` — UC1 through UC4.
...
- The immutable contract is anchored in `docs/product-defining-v2.0/immutable-fields.md`.
- The block-type and hybrid-extraction contract is anchored in `docs/product-defining-v2.0/blocks.md`.
...
These rules are load-bearing and must match `docs/product-defining-v2.0/immutable-fields.md`.
...
This section restates the block-type contract from `docs/product-defining-v2.0/blocks.md` to remove implementation ambiguity.
...
Status note:
- The immutable field names and shapes are canonical per `docs/product-defining-v2.0/immutable-fields.md`.
...
Proposed values (derived from `docs/product-defining-v2.0/blocks.md` accepted types + the explicitly-supported `.md` path):

- `md`
- `txt` *(not listed in the Docling upload table in `blocks.md`, but implied by the text-native `source_total_characters` note in `immutable-fields.md` and by legacy docs; include or explicitly reject)*
- `docx`
- `pptx`
- `pdf`
- `html`
...
Canonical suggested values (as listed in `docs/product-defining-v2.0/immutable-fields.md`):
...
Canonical strict list (restated from `docs/product-defining-v2.0/blocks.md`):
...
Canonical examples (as listed in `docs/product-defining-v2.0/immutable-fields.md`):
...
These rules must match `docs/product-defining-v2.0/immutable-fields.md`. They are repeated here for Section 10 self-containment.
...
These are not platform enums, but they are listed in `docs/product-defining-v2.0/blocks.md` and are commonly needed when writing normalization code.

DoclingDocument value-sets (as listed in `blocks.md`):
...
mdast node types (as listed in `blocks.md`):
