...
With blocks, the platform handles the splitting deterministically (mdast parsing), each paragraph gets the same schema and the same Strunk reference, workers process independently, and the writer gets a JSONL file where reassembly is mechanical (sort by `block_index`). The 39,000-word manuscript is no harder than a 500-word essay — only the number of blocks changes.
