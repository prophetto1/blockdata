...
description: Use when a live webpage needs a deterministic viewport-specific measurement pass. Trigger for requests to screenshot a URL, extract rendered layout or computed styles, create a parity spec, or build a similar page from measured browser output instead of manual visual guessing.
---
...
```bash
node scripts/measure-layout.mjs --url <url-or-file> --width 1440 --height 1024 --output-dir <dir> --json-out <file>
```
...
- shell measurements
- hero measurements
- first visible search input if present
- card count plus first visible card
- visible section summaries for `header`, `nav`, `main`, `section`, `aside`, and `footer`
...
- The script does not inspect hidden menus, modals, hover states, or other unopened UI unless the user explicitly asks for another captured state.
...
- another viewport
- another page state
- a named component not covered by the baseline capture

If you add follow-up measurements, label them as an additional pass instead of mixing them into the baseline.
