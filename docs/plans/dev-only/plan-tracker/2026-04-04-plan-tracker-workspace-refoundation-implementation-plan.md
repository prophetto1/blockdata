...
|------|------|--------------|
| `usePlanTracker` | `web/src/pages/superuser/usePlanTracker.tsx` | Rebase orchestration on the workspace-editor document/session contract, add lifecycle-state filtering, and drive a metadata-derived left navigator plus structured right-column inspector |
...
|------|------|--------------|
| `planTrackerModel` | `web/src/pages/superuser/planTrackerModel.ts` | Lock lifecycle states, artifact types, metadata schema, legacy normalization rules, and grouped plan/artifact semantics for the refounded shell |
...
|-----------|------|--------------|
| `PlanMetadataPane` | `web/src/pages/superuser/PlanMetadataPane.tsx` | Keep the right metadata/actions pane but align it to the refounded three-column shell and the locked Summary -> Classification -> Timeline -> Workflow -> Notes -> Related Artifacts inspector order |
...
| Test | File | Covers |
|------|------|--------|
| `planTrackerModel.test.ts` | `web/src/pages/superuser/planTrackerModel.test.ts` | Lifecycle normalization, metadata schema, artifact grouping, deterministic naming |
| `usePlanTracker.test.tsx` | `web/src/pages/superuser/usePlanTracker.test.tsx` | Directory load, metadata-driven filtering, document open/save, dirty-action gating, lifecycle actions, and artifact-backed notes |
...
**Reused as-is in this refoundation**
...
**Step 1:** Lock the lifecycle-state taxonomy, artifact-type taxonomy, and required metadata schema used by the tracker.
...
**Step 4:** Add helpers that bucket plan units into the locked lifecycle-state tabs and expose the required metadata fields consistently.
**Step 5:** Keep deterministic artifact ordering and the distinction between selected plan unit and selected artifact.
**Step 6:** Write failing tests for lifecycle normalization, metadata schema enforcement, grouping, and filename determinism.
**Step 7:** Make the tests pass.

**Test command:** `cd web && npm run test -- src/pages/superuser/planTrackerModel.test.ts`
**Expected output:** Vitest reports the tracker-model tests passing, including lifecycle normalization, artifact typing, metadata schema handling, and grouping behavior.
...
**Step 3:** Keep the metadata model browser-local by reading actual files from `docs/plans/**`, parsing metadata, grouping plan units, and filtering into the locked lifecycle-state tabs.
...
**Step 6:** Expose the right-column inspector data in the locked section order and ensure the required metadata fields are available to the pane.
**Step 7:** Add tests for initial load, lifecycle filtering, selected artifact open/save, metadata availability, and pending-action resolution.

**Test command:** `cd web && npm run test -- src/pages/superuser/usePlanTracker.test.tsx`
**Expected output:** Vitest reports the refounded hook tests passing, including lifecycle filtering, document save behavior, right-pane metadata availability, and dirty-action gating.
...
**Step 1:** Replace the older action vocabulary with the locked lifecycle-transition actions in the right column: `Send Back`, `Approve`, `Mark Implementing`, `Mark Implemented`, `Request Verification`, and `Close`.
**Step 2:** Add the structured Notes / Action Composer area that creates artifact-backed note files rather than ephemeral inline comments.
**Step 3:** Ensure each action still writes or updates concrete Markdown artifacts on disk and updates the controlling lifecycle state correctly.
**Step 4:** Keep the locked dirty-action gate in front of all workflow side effects.
**Step 5:** Refresh the metadata-driven navigator after each workflow write without re-picking the directory.
**Step 6:** Keep plan-unit selection stable while updating the active artifact after action-driven note or state changes.
**Step 7:** Add failing tests for each lifecycle-action flow and note-composer flow in the refounded shell, then make them pass.

**Test command:** `cd web && npm run test -- src/pages/superuser/usePlanTracker.test.tsx`
**Expected output:** Workflow tests pass, including dirty gating, lifecycle state updates, artifact-backed note creation, and selection persistence.
...
**Step 5:** Verify the right column follows the locked inspector order and shows the required metadata fields.
**Step 6:** Fix any contract drift discovered during verification without expanding scope.
