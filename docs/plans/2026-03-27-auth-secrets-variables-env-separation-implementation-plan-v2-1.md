# Auth, Secrets, Variables, and Env Separation Implementation Plan (v2.1)
...
- **Author:** v2.1 revision (corrects v2 runtime seam, worker identity, and verification defects)
- **Date:** 2026-03-27
...
This v2.1 plan preserves the async secret-resolution seam, adds the missing worker identity plumbing as an explicit compatibility-surface change, and re-locks the inventory and verification sections so the plan can be executed without improvisation.
...
| model_registry.py | decrypt | user_api_keys | user-api-keys-v1 | 288 |
...
| Verb | Path | Action | Status |
| --- | --- | --- | --- |
| POST | /{function_name} | Execute a worker plugin by function name; request body adds optional `user_id` for secret resolution compatibility | Existing route shape preserved; request contract modified |

#### POST /{function_name} (pipeline-worker compatibility surface)

- **Auth:** existing worker auth behavior unchanged
- **Request:** existing `PluginRequest` plus optional `user_id?: string`
- **Response:** existing `PluginResponse` unchanged
- **Touches:** `public.user_variables` only when `user_id` is present and runtime secret resolution is invoked
- **Change:** request body adds optional `user_id?: string`, and the existing route passes that value into `ExecutionContext`.
- **Why:** worker-side `get_secret()` cannot satisfy the locked runtime resolution contract during the dual-run period unless user identity is carried into the existing execution context.
...
- pipeline-worker remains a deprecated compatibility surface. This plan does not add worker routes, but it does add optional `user_id` propagation to the existing `POST /{function_name}` request so worker-side secret resolution can satisfy the same runtime contract during the decommissioning window.
...
- ExecutionContext.get_secret() remains async in both platform-api and pipeline-worker, resolves user-scoped secrets when a user_id is present, and falls back to env only when no user secret exists.
...
**Modified existing compatibility endpoint surfaces:** 1

- POST /{function_name} (pipeline-worker) - request body adds optional `user_id`; route path and response envelope stay unchanged
...
- New route modules: 1
- Modified existing backend modules: 10
...
- services/pipeline-worker/tests/test_auth.py
...
- `ExecutionContext.get_secret()` remains `async def` in both runtimes.
- `pipeline-worker` continues to expose the existing `POST /{function_name}` execution route; only the request body grows an optional `user_id`.
- `pipeline-worker.app.shared.auth._resolve_value()` remains async and must `await context.get_secret(...)` for `$SECRET.*` resolution.
- Worker requests without `user_id` preserve current env-only fallback behavior.
...
- **Step 3:** Add test that create calls get_envelope_key() not os.environ.get("SUPABASE_SERVICE_ROLE_KEY").
- **Step 4:** Update test_variables.py to lock the deprecation-alias behavior for /variables.
- **Step 5:** Run and confirm new tests fail.
...
- **Step 1:** Create secrets.py with the locked request/response models and metadata-only CRUD handlers. Use get_envelope_key() for encrypt and decrypt_with_fallback() for any decrypt. Add the locked trace spans and metrics.
...
### Task 7: Implement trusted runtime secret resolution

**File(s):** `services/platform-api/app/domain/plugins/models.py`, `services/pipeline-worker/app/shared/context.py`, `services/pipeline-worker/app/shared/auth.py`, `services/pipeline-worker/app/main.py`, `services/platform-api/tests/test_plugins.py`, `services/pipeline-worker/tests/test_auth.py`, `services/pipeline-worker/tests/conftest.py`
...
async def get_secret(self, name: str) -> str:
    ...
    if self.user_id and self.supabase_url and self.supabase_key:
        ...
        result = sb.table("user_variables").select(
            "value_encrypted"
        ).eq("user_id", self.user_id).eq("name", name).limit(1).execute()
...
- **Step 2:** Add `user_id: str = ""` to `pipeline-worker` `ExecutionContext`, keep `get_secret()` async there too, and implement the same dual-key user-secret lookup with env fallback when `user_id` is absent.
- **Step 3:** Add optional `user_id: str = ""` to `pipeline-worker` `PluginRequest` and pass it into `ExecutionContext` inside `pipeline-worker/app/main.py` so the existing worker route can carry identity without changing its path or response shape.
- **Step 4:** In `pipeline-worker/app/shared/auth.py`, add `$SECRET.NAME` resolution to `_resolve_value` and await the async secret lookup:

```python
if value.startswith("$SECRET.") and context:
    secret_name = value[8:]
    return await context.get_secret(secret_name)
```

- **Step 5:** Add the `secrets.resolve` trace span and `platform.secrets.resolve.count` metric to both `get_secret()` implementations.
- **Step 6:** Extend `services/platform-api/tests/test_plugins.py` for async secret hit and env fallback coverage, add `services/pipeline-worker/tests/test_auth.py` for `$SECRET.*` resolution with and without `user_id`, and update `services/pipeline-worker/tests/conftest.py` only if needed to support the new optional context field.

**Test command:**

```bash
python -m pytest services/platform-api/tests/test_plugins.py services/platform-api/tests/test_connections.py services/pipeline-worker/tests/test_auth.py -v
```

**Expected output:** Runtime contexts resolve user secrets first, env fallback second, and worker auth resolution preserves env-only behavior when `user_id` is omitted.
...
```bash
python -m pytest services/platform-api/tests/test_secrets.py services/platform-api/tests/test_variables.py services/platform-api/tests/test_connections.py services/platform-api/tests/test_plugins.py services/platform-api/tests/infra/test_crypto.py services/pipeline-worker/tests/test_auth.py -v
```
...
- **Step 5:** Verify the worker compatibility seam still accepts the existing `POST /{function_name}` route shape while honoring optional `user_id` for `$SECRET.*` resolution.
