...
| JSONL export (inert annotation)  | Export assembles from `blocks JOIN documents ORDER BY block_index`; annotation placeholder: `{schema_ref: null, schema_uid: null, data: {}}` |
...
| Capability                                 | Grounded In                                                                                             |
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------- |
| User-defined annotation schemas            | `schemas` table — `schema_id` UUID PK, `owner_id` UUID, `schema_ref` slug (unique per owner), `schema_uid` SHA256 (unique per owner), `schema_jsonb` JSONB |
| Annotation runs                            | `annotation_runs` table — `run_id` UUID PK, FK to `documents.doc_uid` + `schemas.schema_id`             |
...
| One-click apply to document   | `annotation_runs` already binds `doc_uid` + `schema_id` → run                 | No change — gallery selection feeds the same "start run" flow                                        |
| Community-contributed schemas | `schema_ref` is a user-facing slug; `schema_uid` is a stable content identity | Add: `author_id` UUID FK to auth.users; RLS policy for public read + owner write                     |
| Schema versioning             | `schema_ref` is currently a flat string (e.g., `strunk_18`)                   | Add: `version` INTEGER column; composite unique on `(owner_id, schema_ref, version)`; `latest` boolean or view |

**Architectural invariant:** A schema is a reusable template. It never contains document-specific data. The same schema definition (`schema_id`) can be applied to any `doc_uid` (and `schema_ref` remains a human-friendly slug).

---

## Tier 3: Integration Infrastructure (Post-Tier 2)

**What it is:** The block inventory and annotation outputs become the hub that external systems connect to. MD-ANNOTATE becomes the document intelligence layer in a larger pipeline.
...
| Multi-model comparison    | Same blocks, same schema, different `run_id` per model                                                                                                   | Already supported: two runs with same `doc_uid` + `schema_id` but different `model_config` produce separate `block_annotations` rows. Frontend toggle between runs on same block view |
...
| Trigger annotation run      | Phase 2 "start run" Edge Function binds `doc_uid` + `schema_id` → populates `block_annotations` | Add: expose as authenticated REST endpoint; return `run_id` for polling                                    |
