# Auth, Secrets, Variables, and Env Separation Implementation Plan (v2)
...
- **Status:** Draft
- **Author:** v2 revision (corrects crypto root contradiction in v1)
- **Date:** 2026-03-27
...
- ExecutionContext.get_secret() in both platform-api and pipeline-worker resolves user-scoped secrets when a user_id is present and falls back to env only when no user secret exists.
...
- New route modules: 1
- Modified existing backend modules: 9
...
- **Step 1:** Add get_secret(name) to ExecutionContext in plugins/models.py:
...
- **Step 2:** Add the same get_secret(name) to ExecutionContext in pipeline-worker/shared/context.py, using the same dual-key decrypt.
- **Step 3:** In auth.py, add $SECRET.NAME resolution to _resolve_value:

```python
if value.startswith("$SECRET.") and context:
    secret_name = value[8:]
    return context.get_secret(secret_name)
```

- **Step 4:** Add the secrets.resolve trace span and platform.secrets.resolve.count metric to both get_secret implementations.
- **Step 5:** Add tests for secret hit, env fallback, and missing secret paths.

**Test command:**

```bash
python -m pytest services/platform-api/tests/test_plugins.py services/platform-api/tests/test_connections.py -v
```

**Expected output:** Runtime contexts resolve user secrets first, env fallback second.
...
```bash
python -m pytest services/platform-api/tests/test_secrets.py services/platform-api/tests/test_variables.py services/platform-api/tests/test_connections.py services/platform-api/tests/test_plugins.py services/platform-api/tests/infra/test_crypto.py -v
```
...
- **Step 5:** Compare actual file inventory against locked counts.
- **Step 6:** Verify platform.crypto.fallback.count metric is registered (will emit once legacy-encrypted rows are decrypted in a live environment).
