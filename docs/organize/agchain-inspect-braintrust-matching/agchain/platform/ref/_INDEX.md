...
1. `../_essentials/platform/inter-step-requirements.md` — **95 numbered requirements** (IS-1.1.1 through IS-7.3.7). Densest requirements document. Covers state management, payload admission, staging/isolation, message assembly, audit, orchestration, post-chain operations.
...
When requirements conflict, **../_essentials/platform/inter-step-requirements.md** takes precedence (most recent, most granular).
...
- Langfuse integration (`../_stale/langfuse-integration.md`) — observability/tracing, not core runner. Moved to `_stale/`; not required for MVP.
