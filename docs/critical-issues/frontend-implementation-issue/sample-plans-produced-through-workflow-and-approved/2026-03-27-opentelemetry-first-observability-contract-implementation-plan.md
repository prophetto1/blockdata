...
- `Settings` gains exactly one runtime header field: `otel_exporter_otlp_headers: dict[str, str] | None` (parsed at config load by `_parse_otlp_headers()` in `config.py`).
...
- `services/platform-api/app/core/config.py` owns OTLP header parsing via `_parse_otlp_headers()`, which validates comma-delimited `key=value` pairs at config load (rejects empty keys, duplicates, and malformed segments as startup errors). `services/platform-api/app/observability/otel.py` owns OTLP exporter creation: it receives the parsed header dict from Settings and threads it through trace, metric, and log exporters. `get_telemetry_status()` preserves its current JSON keys and never exposes header values.
