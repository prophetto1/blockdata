...
KESTRA_JAVA_ROOT_ENV = os.environ.get("KESTRA_JAVA_ROOT") or os.environ.get("KESTRA_ROOT")
KESTRA_JAVA_ROOT = Path(KESTRA_JAVA_ROOT_ENV) if KESTRA_JAVA_ROOT_ENV else None
    ...
    with open(py_path) as f:
        for line in f:
            ...
            if m:
                ...
                if KESTRA_JAVA_ROOT is not None:
                    remapped = re.sub(
                        r"^[A-Za-z]:/KESTRA/",
                        str(KESTRA_JAVA_ROOT).replace("\\", "/").rstrip("/") + "/",
                        normalized,
                    )
...
def main():
    ...
    if KESTRA_JAVA_ROOT is None:
        print("Warning: KESTRA_JAVA_ROOT/KESTRA_ROOT is not set; Java source lookups may be skipped.")
