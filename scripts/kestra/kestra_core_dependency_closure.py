...
SEED_ROOT = Path(os.environ.get("KESTRA_SEED_ROOT", PROJECT_ROOT / "core" / "kestra-ref"))
CORE_ROOT_ENV = os.environ.get("KESTRA_CORE_ROOT")
MODEL_ROOT_ENV = os.environ.get("KESTRA_MODEL_ROOT")
CORE_ROOT = Path(CORE_ROOT_ENV) if CORE_ROOT_ENV else None
MODEL_ROOT = Path(MODEL_ROOT_ENV) if MODEL_ROOT_ENV else None
OUTPUT_ROOT = Path(
    os.environ.get("KESTRA_OUTPUT_ROOT", PROJECT_ROOT / "output" / "kestra-core-dependency-closure")
)
...
def main() -> None:
    if CORE_ROOT is None or MODEL_ROOT is None:
        raise SystemExit(
            "Set KESTRA_CORE_ROOT and KESTRA_MODEL_ROOT before running this script."
        )
