...
**Outputs:** Chunked block set (may produce new derived block_uids via `(conv_uid, chunk_strategy, range)`)
...
**Contract:** Enrichment steps are additive — they never modify the original extraction overlay. They produce new overlay fields or separate enrichment records linked to block_uids.
...
**Contract:** The export boundary is confirmed, schema-conformant structured records with full provenance (immutable envelope + user_defined overlay). Every record traces to its source.
...
| `reindex-block-positions` | Recalculate block_index after merges, splits, or deletions | python |
...
| Action | Description | Runtime |
|--------|-------------|---------|
| `llm-extract-schema-fields` | Fill schema fields per block using LLM with prompt_config | llm-api |
...
| `push-to-neo4j` | Push fields to Neo4j as nodes and edges per graph_mapping | typescript |
