...
function makeSupabaseForExistingDoc(existing: ExistingDocRow | null, convUid: string | null) {
  return {
    from(table: string) {
      ...
      if (table === "conversion_parsing") {
        return {
          ...
          async maybeSingle() {
            return { data: convUid ? { conv_uid: convUid } : null, error: null };
          },
        };
      }
...
Deno.test("checkIdempotency returns existing doc when same project uploads identical file", async () => {
  ...
  const supabaseAdmin = makeSupabaseForExistingDoc(existing, "conv-1");
