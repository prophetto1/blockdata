...
function createWorkspaceNotBoundError(workspacePath) {
  const error = new Error(`Workspace path is not bound to a discussion projection: ${workspacePath}`);
  error.code = 'workspace_not_bound';
  return error;
}

async function resolveDiscussionTaskId({ args, discussionBucket }) {
  if (args['task-id']) {
    return String(args['task-id']);
  }

  const workspacePath = args['workspace-path'];
  if (!workspacePath) {
    throw new Error('One of --task-id or --workspace-path is required');
  }
  ...
  if (!resolved) {
    throw createWorkspaceNotBoundError(String(workspacePath));
  }
...
async function runIdentityClaim(args) {
  ...
  printJson({
    runtimeIdentity: {
      host: identity.host,
      agentId: result.identity,
    },
    claim: result,
  });
}

async function runIdentityRelease(args) {
  const fallbackIdentity = identityFromArgs(args);
  const identity = requireOption(args, 'identity');

  const result = await withBus(args, async (bus) => {
    const { presenceBucket } = await openCoordinationBuckets({ bus });
    return releaseAgentIdentity({
      presenceBucket,
      host: fallbackIdentity.host,
      identity,
    });
  });

  printJson({
    runtimeIdentity: fallbackIdentity,
    release: result,
  });
}

async function runIdentityList(args) {
  const identity = identityFromArgs(args);

  const result = await withBus(args, async (bus) => {
    const { presenceBucket } = await openCoordinationBuckets({ bus });
    return listAgentIdentities({
      presenceBucket,
      host: identity.host,
      family: args.family ? String(args.family) : undefined,
      includeStale: String(args['include-stale'] ?? 'true').toLowerCase() !== 'false',
    });
  });
...
async function runDiscussionPost(args) {
  const identity = identityFromArgs(args);
  const eventKind = String(args['event-kind'] ?? 'message_posted');
  const payload = parseJson(args.json, {});

  const result = await withBus(args, async (bus) => {
    const { stateBucket, discussionBucket } = await openCoordinationBuckets({ bus });
    const taskId = await resolveDiscussionTaskId({ args, discussionBucket });
    let normalizedPayload = payload;
    if (eventKind === 'message_posted' && !normalizedPayload.author) {
      normalizedPayload = { ...normalizedPayload, author: identity };
    }

    if (
      eventKind === 'response_requested'
      && !normalizedPayload.recipients
      && !normalizedPayload.recipient
      && (args['recipient-agent-id'] || args['recipient-host'])
    ) {
      normalizedPayload = {
        ...normalizedPayload,
        recipients: [{
          host: String(args['recipient-host'] ?? identity.host),
          agentId: String(args['recipient-agent-id'] ?? identity.agentId),
        }],
      };
    }

    const discussion = await projectDiscussionEvent({
      stateBucket,
      discussionBucket,
      taskId,
      eventKind,
    ...
    const publication = await publishTaskEvent({
      bus,
      taskId,
      eventKind,
      payload: normalizedPayload,
...
async function runDiscussionAck(args) {
  const identity = identityFromArgs(args);
  const payload = parseJson(args.json, {});
  ...
  const result = await withBus(args, async (bus) => {
    const { stateBucket, discussionBucket } = await openCoordinationBuckets({ bus });
    const taskId = await resolveDiscussionTaskId({ args, discussionBucket });
    const normalizedPayload = payload.recipient
      ? payload
      : { ...payload, recipient: identity };
...
async function runDiscussionList(args) {
  const workspacePath = args['workspace-path'] ? String(args['workspace-path']) : undefined;
  ...
  const result = await withBus(args, async (bus) => {
    ...
    if (workspacePath) {
      ...
      if (!resolved) {
        throw createWorkspaceNotBoundError(workspacePath);
      }
    }

    return listDiscussionStates({
      stateBucket,
      discussionBucket,
...
async function runDiscussionCheckStale(args) {
  const identity = identityFromArgs(args);
  const workspacePath = args['workspace-path'] ? String(args['workspace-path']) : undefined;
  ...
  const result = await withBus(args, async (bus) => {
    ...
    return markStaleDiscussions({
      stateBucket,
      discussionBucket,
      taskId: args['task-id'] ? String(args['task-id']) : undefined,
      workspacePath,
      staleAfterSeconds: parseIntegerOption(args['stale-after-seconds'], 300),
      publishEvent: async ({ taskId, eventKind, payload }) => publishTaskEvent({
        bus,
        taskId,
        eventKind,
        payload,
...
const commands = new Map([
  ...
  ['discussion-post', runDiscussionPost],
  ['discussion-ack', runDiscussionAck],
  ['discussion-list', runDiscussionList],
  ['discussion-check-stale', runDiscussionCheckStale],
  ['identity-claim', runIdentityClaim],
  ['identity-release', runIdentityRelease],
  ['identity-list', runIdentityList],
