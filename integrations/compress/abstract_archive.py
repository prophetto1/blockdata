from __future__ import annotations

# Source: E:\KESTRA-IO\plugins\plugin-compress\src\main\java\io\kestra\plugin\compress\AbstractArchive.java
# WARNING: Unresolved types: ArchiveException, ArchiveInputStream, ArchiveOutputStream, CompressionAlgorithm, InputStream, OutputStream

from abc import ABC, abstractmethod
from dataclasses import dataclass
from enum import Enum
from typing import Any

from integrations.compress.abstract_task import AbstractTask
from integrations.compress.archive_decompress import ArchiveDecompress
from engine.core.exceptions.illegal_variable_evaluation_exception import IllegalVariableEvaluationException
from engine.core.models.property.property import Property
from engine.core.runners.run_context import RunContext


@dataclass(slots=True, kw_only=True)
class AbstractArchive(ABC, AbstractTask):
    algorithm: Property[ArchiveAlgorithm]
    compression: Property[ArchiveDecompress.CompressionAlgorithm] | None = None

    def archive_input_stream(self, input_stream: InputStream, run_context: RunContext) -> ArchiveInputStream:
        raise NotImplementedError  # TODO: translate from Java

    def archive_output_stream(self, output_stream: OutputStream, run_context: RunContext) -> ArchiveOutputStream:
        raise NotImplementedError  # TODO: translate from Java

    class ArchiveAlgorithm(str, Enum):
        AR = "AR"
        ARJ = "ARJ"
        CPIO = "CPIO"
        DUMP = "DUMP"
        JAR = "JAR"
        TAR = "TAR"
        ZIP = "ZIP"
